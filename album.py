...
def save(year, artist, genre, album):
    """
    Сохраняет в базу данные об альбоме
    """
    assert isinstance(year, int), "Некорректная дата"
    assert isinstance(artist, str), "Некорректный артист"
    ...
    if saved_album is not None:
        raise AlreadyExists("Такой альбом существует под номером #{}".format(saved_album.id))
